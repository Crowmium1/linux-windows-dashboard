...
TEST_MODE=1
TEST_DIR="${TEST_BASE_DIR}/collect_test_$(date +%Y%m%d_%H%M%S)"
...
setup() {
    ...
    mkdir -p "$TEST_BASE_DIR/var/data/system_info"
...
SSH_KEY_DIR="$TEST_BASE_DIR/ssh"
SSH_CONFIG_FILE="$TEST_BASE_DIR/ssh/config"
SSH_KNOWN_HOSTS="$TEST_BASE_DIR/ssh/known_hosts"
EOL

    # Create SSH test directory
    mkdir -p "$TEST_BASE_DIR/ssh"
    chmod 700 "$TEST_BASE_DIR/ssh"
}
...
test_ssh_info() {
    ...
    mkdir -p "$TEST_BASE_DIR/ssh"
    touch "$TEST_BASE_DIR/ssh/config"
    touch "$TEST_BASE_DIR/ssh/known_hosts"
    
    # Test SSH config collection
    run_test "collect SSH config" "cp '$TEST_BASE_DIR/ssh/config' '$TEST_LOG_DIR/ssh_config.log'" || ((failures++))
    
    # Test SSH known hosts collection
    run_test "collect known hosts" "cp '$TEST_BASE_DIR/ssh/known_hosts' '$TEST_LOG_DIR/known_hosts.log'" || ((failures++))

...
TEST_MODE=1
TEST_DIR="${TEST_BASE_DIR}/error_test_$(date +%s)"
...
setup() {
    ...
    mkdir -p "$TEST_BASE_DIR/data/error_handling"
    mkdir -p "$TEST_LOG_DIR"
    mkdir -p "$TEST_BASE_DIR/tmp"
...
DATA_DIR="$TEST_BASE_DIR/data/error_handling"
...
test_permission_denied() {
    ...
    if is_wsl; then
        ...
        local readonly_dir="$TEST_BASE_DIR/readonly"
        ...
        local test_script="$TEST_BASE_DIR/test.sh"
...
test_disk_space() {
    ...
    local test_file="$TEST_BASE_DIR/large_file.txt"
    
    # In WSL, we'll test disk space check instead of actual disk full
    run_test "check available space" "df -h $TEST_BASE_DIR >/dev/null" 0
